...
export default async function TrackerPage({ params }: TrackerPageProps) {
  ...
  return (
    <div className="min-h-screen text-cw-ink font-sans">
      <div className="cw-shell">
        <header className="cw-header">
          <div className="cw-container py-5 flex items-center justify-between">
            ...
            >
              <span className="text-cw-gold">TEL</span> (972) 945-1610
            </a>
          </div>
        </header>
        ...
        <div className="cw-container py-10 md:py-16">
          <div className="max-w-5xl mx-auto">
            ...
            <section className="mt-8 grid lg:grid-cols-[1fr_.9fr] gap-6">
              <div className="cw-panel p-6">
                <div className="cw-eyebrow mb-2">Secure Portal</div>
                <h2 className="font-display text-2xl text-cw-navy">Need document access?</h2>
                <p className="text-sm text-cw-ink-soft mt-2">
                  Request a signed portal link using the email already on file. The firm can then
                  coordinate uploads and signature packets inside the private workspace.
                </p>
                <div className="mt-4">
                  <PortalAccessForm leadId={lead.id} />
                </div>
              </div>

              <div className="cw-panel p-6">
                <div className="cw-eyebrow mb-2">Quick Status</div>
                <dl className="space-y-3 text-sm">
                  <div>
                    <dt className="text-[11px] uppercase tracking-widest text-cw-ink-mute font-semibold">Lead ID</dt>
                    <dd className="font-mono text-xs text-cw-navy mt-1 break-all">{lead.id}</dd>
                  </div>
                  <div>
                    <dt className="text-[11px] uppercase tracking-widest text-cw-ink-mute font-semibold">Stage</dt>
                    <dd className="text-cw-navy mt-1">{lead.status.replace(/_/g, ' ')}</dd>
                  </div>
                  <div>
                    <dt className="text-[11px] uppercase tracking-widest text-cw-ink-mute font-semibold">Phone</dt>
                    <dd className="text-cw-navy mt-1">{lead.phone}</dd>
                  </div>
                </dl>
              </div>
            </section>
