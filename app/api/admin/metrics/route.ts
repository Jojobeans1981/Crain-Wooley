...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const qualified = await prisma.lead.count({ where: { qualified: true } })
    ...
    const scheduled = await prisma.lead.count({
      where: { status: { in: scheduledStatuses as any } },
    })

    const hired = await prisma.lead.count({ where: { status: 'HIRED' } })
