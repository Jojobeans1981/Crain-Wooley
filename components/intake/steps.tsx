...
export function CounselFinalStep({
  ...
  if (stepId === 'about') {
    ...
    return (
      <div style={isMobile
        ...
        <Field label="Email address" required error={has('email')} errorText="enter a valid email" span={4}>
        ...
        <Field label="Phone" required error={has('phone')} errorText="enter a 10-digit phone" span={2}>
        ...
        <Field label="ZIP code" required error={has('zip')} errorText="enter a 5-digit ZIP" hint="Routes you to your nearest office." span={2}>
        ...
        {isProbate && (
          <Field label="Do you live in Texas?" required error={has('clientLivesInTexas')} group
  ...
  if (stepId === 'family') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        <Field label="Marital status" required error={has('maritalStatus')} group>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(5, 1fr)', gap: 8 }}>
            {statuses.map(([id, l]) => (
              <RadioCard key={id} narrow name="maritalStatus" value={id} checked={form.maritalStatus === id}
        ...
        <div style={isMobile
          ...
          <Field label="Do you have children?" required error={has('hasChildren')} group>
        ...
        {isWills && form.hasChildren === true && (
          <Field label="Any children under 18?" group hint="Determines whether we set up a guardian / minor's trust.">
        ...
        <Field label="Do you provide for a dependent with special needs?" group hint="A Supplemental Needs Trust may be appropriate.">
  ...
  if (stepId === 'estate') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        <Field label="Approximate total assets" required error={has('assetRange')} group
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(1, 1fr)' : 'repeat(3, 1fr)', gap: 8 }}>
            {brackets.map(a => (
              <RadioCard key={a.id} narrow name="assetRange" value={a.id} checked={form.assetRange === a.id} onClick={() => update({ assetRange: a.id })} label={a.label} />
            ))}
          </div>
        </Field>
        <div style={isMobile
          ...
          <Field label="Own real estate?" required error={has('ownsRealEstate')} group>
          ...
          <Field label="Own a business?" required error={has('ownsBusiness')} group>
          ...
          <Field label="Existing estate plan?" group hint="Will, trust, or POA already in place.">
        ...
        <Reveal open={form.ownsRealEstate === true}>
          <div style={{ padding: '18px 20px', background: cf.ivoryWarm, borderLeft: `2px solid ${cf.brass}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Field label="Attach a deed (optional)" group hint="Stored locally for the demo — not transmitted.">
        ...
        <Reveal open={form.ownsBusiness === true}>
          <div style={{ padding: '18px 20px', background: cf.ivoryWarm, borderLeft: `2px solid ${cf.brass}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={isMobile
              ...
              <Field label="Succession plan in place?" required={form.ownsBusiness === true} error={has('businessHasSuccession')} group>
            ...
            <Field label="Attach company documents (optional)" group hint="Certificate of Formation, Operating Agreement, etc.">
  ...
  if (stepId === 'wishes') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        ...
        <Field label="Living Will (Directive to Physicians) — you" group
        ...
        {isPartnered && (
          <Field label={`Living Will — your ${form.maritalStatus === 'married' ? 'spouse' : 'partner'}`} group
  ...
  if (stepId === 'additional') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        {questions.map(([key, q]) => (
          <Field key={key} label={q} group>
            <YesNo value={form[key] as boolean | null} onChange={v => update({ [key]: v })} />
          </Field>
        ))}
        <Field label="Anything else you would like to share or attach?" group
  ...
  if (stepId === 'services') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        <Field label="Select all that apply" required error={has('services')} group
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 8 }}>
            {SERVICE_OPTIONS.map(s => (
              <CheckCard key={s.id} value={s.id} checked={form.services.includes(s.id)} onClick={() => toggle(s.id)} label={s.label} note={s.note} />
            ))}
          </div>
        </Field>
  ...
  if (stepId === 'schedule') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        <Field label="Preferred office" required error={has('preferredOffice')} group>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            {OFFICE_OPTIONS.map(o => (
              <RadioCard key={o.id} name="preferredOffice" value={o.id} checked={form.preferredOffice === o.id} onClick={() => update({ preferredOffice: o.id })} label={o.label} sub={o.sub} />
            ))}
          </div>
        </Field>
        <div style={isMobile
          ...
          <Field label="Consultation format" required error={has('consultationFormat')} group>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {formats.map(f => (
                <RadioCard key={f.id} narrow name="consultationFormat" value={f.id} checked={form.consultationFormat === f.id} onClick={() => update({ consultationFormat: f.id })} label={f.label} sub={f.sub} />
              ))}
            </div>
          </Field>
          <Field label="Best time of day" group>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {times.map(t => (
                <RadioCard key={t.id} narrow name="preferredContactTime" value={t.id} checked={form.preferredContactTime === t.id} onClick={() => update({ preferredContactTime: t.id })} label={t.label} sub={t.sub} />
              ))}
            </div>
          </Field>
        </div>
        <Field label="Timeline" required error={has('urgency')} group>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
            {URGENCY_OPTIONS.map(u => (
              <RadioCard key={u.id} name="urgency" value={u.id} checked={form.urgency === u.id} onClick={() => update({ urgency: u.id })} label={u.label} sub={u.note} />
            ))}
          </div>
        </Field>
        <Field label="Anything else we should know?" hint="Optional. Recent loss, pending sale, family complexity — anything that helps us prepare.">
          <textarea value={form.notes} onChange={e => update({ notes: e.target.value })} rows={3} className="cw-fc"
  ...
  if (stepId === 'decedent') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        ...
        <Field label="Was the decedent a Texas resident?" required error={has('decedentTexasResident')} group
        ...
        <Field label="Did the decedent have a Trust?" required error={has('decedentHadTrust')} group
        ...
        <Field label="Your relationship to them" required error={has('decedentRelationship')} group>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 8 }}>
            {relationships.map(([l, v]) => (
              <RadioCard key={v} narrow name="decedentRelationship" value={v} checked={form.decedentRelationship === v}
  ...
  if (stepId === 'probate-estate') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        ...
        <Field label="Approximate total DEBTS" required error={has('debtRange')} group
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(1, 1fr)' : 'repeat(3, 1fr)', gap: 8 }}>
            {ASSET_OPTIONS.map(a => (
              <RadioCard key={a.id} narrow name="debtRange" value={a.id} checked={form.debtRange === a.id} onClick={() => update({ debtRange: a.id })} label={a.label} />
            ))}
          </div>
        </Field>
        <div style={isMobile
          ...
          <Field label="Owned real estate?" required error={has('ownsRealEstate')} group>
            <YesNo value={form.ownsRealEstate} onChange={v => update({ ownsRealEstate: v, realEstateCount: v ? form.realEstateCount : '', realEstateProperties: v ? form.realEstateProperties : [] })} error={has('ownsRealEstate')} />
          </Field>
          <Field label="Owned a business?" required error={has('ownsBusiness')} group>
            <YesNo value={form.ownsBusiness} onChange={v => update({ ownsBusiness: v, businessType: v ? form.businessType : '' })} error={has('ownsBusiness')} />
          </Field>
          <Field label="Out-of-state property?" required error={has('ownsOutOfState')} group
  ...
  if (stepId === 'will-filings') {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
        <Field label="Did they sign a will?" required error={has('willExists')} group>
        ...
        <Reveal open={form.willExists === 'yes'}>
          <div style={{ padding: '18px 20px', background: cf.ivoryWarm, borderLeft: `2px solid ${cf.brass}`, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Field label="Do you have the original document?" group
        ...
        <Field label="Has anything been filed in probate court yet?" required error={has('probateCourtFiled')} group>
          <TriOption options={[['Yes','yes'],['No','no'],['Not sure','unsure']]}
        ...
        <Reveal open={form.probateCourtFiled === 'yes'}>
          <div style={isMobile
            ...
            <Field label="Executor appointed?" group>
              <YesNo value={form.probateExecutorAppointed}
                onChange={v => update({ probateExecutorAppointed: v })} />
            </Field>
          </div>
        </Reveal>
        <Field label="Are there known disputes among heirs?" group
